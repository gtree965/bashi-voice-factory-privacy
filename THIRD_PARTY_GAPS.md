...
|---|---|---|---|
| G01 | `vulkan_backend_spike/Qwen3-TTS-GGUF/qwen3_tts_gguf/inference/*` 及随包 readme/requirements | 固定 dc8950d 树未找到根许可证；其中 27 个随包文件有 1 个修改，嵌套第三方 LICENSE 不能证明整个转换运行时授权 | 提供覆盖该版本代码、文档和修改分发的许可/授权依据；不能制造 LICENSE。未联系上游，也未改写代码 |
...
- **页脚文案：** “个人和教育用途均可免费使用”没有出现“仅限”或禁止商业用途，不能仅凭这句话认定与 MIT 直接冲突。它容易让用户误解范围，建议后续改为“本项目代码采用 MIT 许可证；第三方组件遵循各自条款”，并核对其他界面/协议是否确有额外限制。本轮不改 UI。
...
优先解决 G01 的转换运行时授权依据；同时准备 G02/G03 的独立打包补正单，避免只有仓库文档而成品仍缺声明。G04 先取得精确条款。模型与 CUDA 继续按原始模型、转换产物、实际下载文件三层追溯。所有涉及联系上游、替换组件、删减文件、重打包或发布的操作均未在本轮执行。
