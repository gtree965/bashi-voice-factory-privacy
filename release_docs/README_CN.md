...
- **长文与跟读流式控制** — 可停止合成并保留已完成片段；默认自动播放首个就绪片段，后续片段生成后自动续播；GGUF 长文试听结束后仍保留可拖动、可下载的完整 MP3。
...
3. 首次启动会：
   ...
   - 启动完成后，命令行窗口会显示访问地址；请在你想用的浏览器里手动打开 `http://127.0.0.1:5050`（应用不会自动打开浏览器）
4. 首次完成后，**之后每次启动都是离线**，启动也会更快（具体用时取决于电脑性能）。
