...
from check_pdf_links import (  # noqa: E402
    AI_TOOL_NAMES, COMMIT_GUARD_EXTRA_AI_NAMES, RETIRED_HOSTS, SESSION_UUID,
    SHARED_MACHINE_RESIDUE, WINDOWS_USER_PATH,
)
...
TRACE_ALLOWLIST = frozenset((
    "scripts/check_pdf_links.py",
    "tests/test_check_pdf_links.py",
...
def staged_changes(root: Path) -> list[tuple[str, str | None]]:
    fields = git(root, "diff", "--cached", "--name-status", "-z",
...
def added_lines(patch: bytes) -> list[str]:
    ...
    for line in patch.splitlines():
        if line.startswith(b"diff --git "):
            ...
            lines.append(line[1:].decode("utf-8", errors="replace"))
    return lines
...
def check_staged(root: Path, terms_path: Path | None = None) -> list[str]:
    ...
    def display(path: str) -> str:
        return "<withheld path>" if sensitive(path) else ascii(path)

    for path in ignored_paths(root, [path for path, _ in changes]):
        issues.append(f"Ignored staged path: {display(path)}. Unstage it; keep local-only files private.")
    for path, old_path in changes:
        label = display(path)
        if sensitive(path):
            issues.append("Local sensitive term in a staged path (path withheld).")
        blob = git(root, "cat-file", "blob", ":" + path)
        paths = [old_path, path] if old_path else [path]
        diff_args = ("diff", "--cached", "--no-ext-diff", "--no-textconv",
        ...
        if binary:
            if sensitive(blob.decode("utf-8", errors="replace")):
                issues.append(f"Local sensitive term in staged binary content: {label}.")
            if path not in TRACE_ALLOWLIST and TRACE_BYTES.search(blob):
                issues.append(f"Disallowed residue in staged binary content: {label}.")
        else:
            patch = git(root, *diff_args, "--no-color", "--unified=0", "--", *paths)
            additions = added_lines(patch)
            if any(sensitive(line) for line in additions):
                issues.append(f"Local sensitive term in added lines: {label}.")
            if path not in TRACE_ALLOWLIST and any(TRACE_TEXT.search(line) for line in additions):
                issues.append(f"Disallowed residue in added lines: {label}.")
    return issues


def check_message(text: str, terms: tuple[str, ...]) -> list[str]:
    """Block agent names, attribution trailers and local terms in a commit message.
    ...
    here. Comment lines are not part of the message, and everything after the
    scissors marker is the diff that ``git commit -v`` appends, so scanning skips
    the former and stops at the latter.
    """
    issues = []
